...
<html class="no-js">
	...
	<body>
		...
		$sql =	'SELECT * FROM node WHERE Lastmessage >= DATE_SUB(NOW(),INTERVAL 14 DAY) AND ID NOT IN ('.implode(', ', $GLOBALS['nodefilter']).') ORDER BY Packets DESC LIMIT 1000';
		if(isset($_REQUEST['admin'])) {
			$sql =	'SELECT * FROM node WHERE Lastmessage >= DATE_SUB(NOW(),INTERVAL 31 DAY) ORDER BY Packets DESC LIMIT 1000';
		}
		//echo $sql;
		$result = mysqlquery($sql);
		echo '<table border="1">';
		echo '<tr><th>Name</th><th>Last</th><th>Packets</th><th>Ch0</th><th>Ch1</th><th>Ch2</th><th>Ch3</th><th>Ch4</th><th>Ch5</th><th>Ch6</th><th>Ch7</th><th>Sf7</th><th>Sf8</th><th>Sf9</th><th>Sf10</th><th>Sf11</th><th>Sf12</th></tr>';
		while ($row = mysqli_fetch_array($result))
		{
			echo '<tr>';
			echo '<td><a href="http://apeldoornindata.nl/data/node.php?id='.$row['Id'].'">';
			if($row['Name'] != '') {
				echo $row['Name'];
			} else {
				echo ' - '.$row['Devid'];
